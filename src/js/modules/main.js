...
import { getEstimatedFlightCost, getEstimatedHotelCost, getEstimatedAttractionCost } from './costApi.js'; // Debes implementar estos módulos
...
async function displayRandomDestinations() {
  ...
  if (!container) return;
  container.innerHTML = '<div class="text-center w-100">Loading images...</div>';
  ...
  // Evento para "Explore": muestra la descripción en el modal
  document.querySelectorAll('.explore-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      e.preventDefault();
      const name = btn.dataset.name;
      const desc = btn.dataset.desc;
      document.getElementById('cityModalLabel').textContent = name;
      document.getElementById('cityModalBody').textContent = desc;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    window.location.href = `src/search_result/searchResult.html?destination=${encodeURIComponent(destination)}&departure=${departureDate}&return=${returnDate}`;
  });
});

// Mostrar costos estimados al seleccionar ciudad
async function showEstimatedCosts(city) {
  ...
  // Evento para mostrar costos cuando cambia el input
  document.getElementById('destination-input').addEventListener('change', (e) => {
    const city = e.target.value;
    showEstimatedCosts(city);
  });
  
  // Submit del formulario: redirige a searchResult.html con los datos por query string
...
async function displayRandomDestinations() {
  ...
  // Evento para "Explore": muestra la descripción y costos en el modal
  document.querySelectorAll('.explore-btn').forEach(btn => {
    btn.addEventListener('click', async e => {
      ...
      // Mostrar costos estimados dentro do modal
